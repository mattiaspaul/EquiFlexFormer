...
def main(args):
    ...
    print('using equi flip resnet model')
    for name, module in model1.named_modules():
        if (isinstance(module,nn.Conv2d) or isinstance(module,nn.ConvTranspose2d)) and module.kernel_size[0]>1:
            before = get_layer(model1, name)
            if(before.kernel_size[0]>1):
                set_layer(model1, name, roto_conv_mean(before))

    if('flip' in args.model):
        print('model1 flip model online in fwd pass defined ')


    optimizer = torch.optim.Adam(list(model1.parameters()),lr=0.001)
    ...
    with tqdm(total=num_iterations, file=sys.stdout) as pbar:
        for i in range(num_iterations):
            ...
            with torch.amp.autocast('cuda',dtype=torch.bfloat16):
                input = model1(x.repeat(1,3,1,1))['out']
                output = model2(input)   
